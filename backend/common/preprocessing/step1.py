...
def parse_header_dd(header_dd):
    """Henter alle metadata fra header dd-listen."""
    metadata_map = {}
...
def load_and_embed_laws(json_folder, embedding_model_name="all-MiniLM-L6-v2",
    ...
    """Laster JSON-filer, lager embeddings, lagrer metadata og embeddings på disk."""
    documents = []
    ...
    # Normaliser for cosine similarity
    ...
    print(f"✅ Lagret {len(documents)} dokumenter og embeddings.")

    # index = faiss.IndexFlatL2(vectors.shape[1])
    index = faiss.IndexFlatIP(vectors.shape[1])
    index.add(vectors)
    faiss.write_index(index, "laws.index")
    print("✅ Faiss-indeks lagret som laws.index")
    return documents
...
# if __name__ == "__main__":
#     # 1. Last dokumenter og lag embeddings
#     documents = load_and_embed_laws("../lovdata_test")
