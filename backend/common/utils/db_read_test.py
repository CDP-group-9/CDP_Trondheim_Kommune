...
DB_CONFIG = {
    ...
    "password": "password",
    "host": "localhost",  # Endre til 'db' hvis du kjører i Docker
    "port": "5433"        # Endre til riktig port som er mappet til host
}
...
def fetch_embeddings(limit=None):
    ...
    try:
        ...
        query = "SELECT law_id, embedding FROM laws"
        if limit:
            query += f" LIMIT {limit}"
